# BridgeRPC — Plano de Implementação (MVP)
...
`qualquer linguagem -> BridgeRPC IR -> qualquer linguagem`
...
3. **Compiler + CLI**
   ...
   - `packages/cli` com `bridge check` e `bridge generate`
...
```txt
Quero criar um projeto chamado BridgeRPC.
...
Arquitetura:
- O centro do projeto deve ser uma IR neutra chamada BridgeRPC IR.
- A lógica deve seguir o fluxo: qualquer linguagem -> BridgeRPC IR -> qualquer linguagem.
...
Implementa primeiro:
1. Tipos da BridgeRPC IR em packages/core.
...
5. CLI com comando bridge check e bridge generate.
...
- Não deixar TypeScript ser o centro conceitual do projeto; o centro deve ser a BridgeRPC IR.
